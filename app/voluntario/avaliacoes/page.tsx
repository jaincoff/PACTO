...
const categoryMeta: Record<string, { icon: LucideIcon; description: string }> = {
  "bem-estar-psicologico": { icon: Heart, description: "Avalia energia, estabilidade emocional, satisfacao pessoal e bem-estar geral." },
  "capacidade-compassiva": { icon: Brain, description: "Avalia a capacidade de reconhecer, compreender e aliviar o sofrimento humano." },
  "habilidades-sociais": { icon: MessageCircle, description: "Avalia competencias de comunicacao, empatia e relacionamento interpessoal." },
  "motivacoes-voluntariado": { icon: Users, description: "Avalia as motivacoes que influenciam a participacao em atividades de voluntariado." },
};

const defaultCategoryMeta = { icon: Sparkles, description: "Avaliacao de competencias para o perfil." };
...
export default function AvaliacoesPage() {
  ...
  const isUnderReview = userStatus === "pending_approval" || userStatus === "pending_admin_approval" || userStatus === "active";
  ...
  useEffect(() => {
    async function load() {
      ...
      if (isUnderReview) {
        try { const data = await getMyAssessmentResults(token); setResults(data); }
        catch (err) { setError(err instanceof Error ? err.message : "Falha ao carregar resultados."); }
        finally { setLoading(false); }
        return;
      }
      try {
        const [questions, prog] = await Promise.all([getSkillTestQuestions(token, currentRole), getTestProgress(token, currentRole)]);
    ...
    load();
  }, [currentRole, token, isUnderReview]);
  ...
  const assessments = useMemo(() => {
    ...
    return grouped.categories.map((cat) => {
      ...
      return { ...cat, status: prog?.status || "not_started", answered: prog?.answered_count || 0, total: cat.questions.length, meta, progCat: prog };
    });
  }, [grouped, progress]);
  ...
  const handleAnswerSelect = (questionId: string, value: string) => {
    ...
    if (token && selectedCategory) {
      ...
      saveTestProgress(token, currentRole, { category: selectedCategory, answers: catAnswers })
    ...
    if (selectedCategory && token) {
      ...
      if (Object.keys(catAnswers).length > 0) {
        ...
        if (!alreadyDone) {
          setLoading(true);
          try {
            result = await saveTestProgress(token, currentRole, { category: selectedCategory, answers: catAnswers });
            setProgress(result);
          } catch {}
          finally { setLoading(false); }
        }
      ...
      <main className="flex-1 overflow-y-auto pt-16 lg:pt-0">
        <div className="mx-auto max-w-5xl space-y-8 p-4 lg:p-8">
          ...
          {step === "overview" && !isUnderReview && (
            <>
              <div>
                ...
                <p className="mt-2 text-muted-foreground">Complete as avaliacoes do seu percurso enquanto voluntario PACTO.</p>
                {loading && <p className="mt-2 text-sm text-muted-foreground">A carregar...</p>}
                {error && <p className="mt-2 text-sm text-destructive">{error}</p>}
              </div>
              <Card className="border-border bg-gradient-to-br from-primary/5 to-primary/10 shadow-sm">
                <CardContent className="flex items-start gap-4 p-6">
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-primary/10"><Heart className="h-6 w-6 text-primary" /></div>
                  <div><h2 className="text-lg font-semibold">Avaliacoes do Voluntario</h2><p className="mt-1 leading-relaxed text-muted-foreground">As avaliacoes permitem acompanhar o seu desenvolvimento pessoal, bem-estar, capacidade compassiva e competencias.</p></div>
                </CardContent>
              </Card>
              <div className="grid gap-6 md:grid-cols-2">
                {assessments.map((a) => {
                  ...
                  return (
                    <Card key={a.key} className={`border-border bg-card shadow-sm ${!isComp ? "cursor-pointer hover:shadow-md" : ""}`}>
                      <CardContent className="flex flex-col p-6">
                        <div className="mb-4 flex items-start justify-between"><div className="flex h-14 w-14 items-center justify-center rounded-xl bg-primary/10"><Icon className="h-7 w-7 text-primary" /></div><span className={`rounded-full px-3 py-1 text-xs font-medium ${status.className}`}>{status.label}</span></div>
                        <h3 className="mb-2 text-lg font-semibold">{a.label}</h3>
                        {isComp && a.progCat?.interpretation ? (
                          <div className="space-y-2">
                            <div className="flex justify-between text-sm"><span className="text-muted-foreground">Pontuacao</span><span className="font-medium">{a.progCat.score}/{a.progCat.max_score} ({a.progCat.percentage}%)</span></div>
                            <div className="h-2 rounded-full bg-secondary"><div className={`h-full rounded-full ${(a.progCat.percentage||0)>=75?"bg-green-500":(a.progCat.percentage||0)>=50?"bg-amber-500":"bg-red-500"}`} style={{width:`${a.progCat.percentage||0}%`}}/></div>
                            <p className="text-sm text-muted-foreground">{a.progCat.interpretation}</p>
                            <span className="text-sm font-medium text-green-600">Completo</span>
                          </div>
                        ) : (
                          <>
                            <p className="mb-4 flex-1 text-sm text-muted-foreground">{a.meta.description}</p>
                            {a.total>0&&<div className="mb-4 h-1.5 rounded-full bg-secondary"><div className={`h-full rounded-full ${isComp?"bg-green-500":"bg-primary"}`} style={{width:`${Math.round((a.answered/a.total)*100)}%`}}/></div>}
                            <div className="flex items-center justify-between">
                              <span className="text-sm text-muted-foreground">{a.answered}/{a.total} perguntas</span>
                              {isComp ? <span className="text-sm font-medium text-green-600">Completo</span> : (
                                <Button className="gap-2 rounded-xl" onClick={() => handleCategorySelect(a.key)}>{hasProg?"Continuar":"Iniciar Avaliacao"}</Button>
                              )}
                            </div>
                          </>
                        )}
                ...
                <Card className="border-border bg-card opacity-70 shadow-sm">
                  <CardContent className="flex flex-col p-6">
                    <div className="mb-4 flex items-start justify-between"><div className="flex h-14 w-14 items-center justify-center rounded-xl bg-muted"><Sparkles className="h-7 w-7 text-muted-foreground"/></div><span className="rounded-full bg-muted px-3 py-1 text-xs font-medium text-muted-foreground">Disponivel em breve</span></div>
                    <h3 className="mb-2 text-lg font-semibold">Valores de Vida</h3><p className="mb-4 flex-1 text-sm text-muted-foreground">Avalia valores pessoais e significado atribuido ao voluntariado.</p>
                    <div className="flex items-center justify-between"><span className="text-sm text-muted-foreground">Em desenvolvimento</span><span className="text-sm font-medium text-muted-foreground">Brevemente</span></div>
                  </CardContent>
                </Card>
              </div>
            </>
          ...
          <Card className="border-border bg-gradient-to-r from-primary/5 to-transparent shadow-sm">
            <CardContent className="flex items-center gap-4 p-6"><Heart className="h-6 w-6 shrink-0 text-primary"/><div><p className="font-medium">O seu desenvolvimento importa.</p><p className="text-sm text-muted-foreground">Cada avaliacao e uma oportunidade de autoconhecimento e crescimento pessoal.</p></div></CardContent>
          </Card>
          )}
        </div>
      </main>
