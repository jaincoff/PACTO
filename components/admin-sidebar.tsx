...
export function AdminSidebar({ activeItem = "painel" }: AdminSidebarProps) {
  ...
  return (
    <aside className="hidden h-screen w-72 flex-col border-r border-border bg-card lg:flex">
      ...
      {/* Profile — clickable */}
      <Link href="/admin/perfil" className="flex flex-col items-center px-6 pb-4 transition-opacity hover:opacity-90">
        <div className="relative h-20 w-20 overflow-hidden rounded-full border-4 border-primary/20">
          <Image src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=200&h=200&fit=crop&crop=face" alt={`Foto de ${displayName}`} fill className="object-cover" />
        </div>
        <h2 className="mt-2 text-base font-semibold text-foreground">{displayName}</h2>
